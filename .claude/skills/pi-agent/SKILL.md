---
name: pi-agent
description: Reference for how Key Lime Pi embeds Pi (@earendil-works/pi-coding-agent) on Ollama - the SDK surface, event shapes, tool schemas, and the Key Lime Pi-specific hooks layered on top. Use when changing anything under apps/electron/src/main/agent/.
---

# Pi Agent Integration

Key Lime Pi's agent is [Pi](https://pi.dev/), embedded via `@earendil-works/pi-coding-agent`
...
| `events.ts` | Pi session events → Key Lime Pi's `StreamChunk` |
...
| `ollama.ts` | Model discovery and `~/.keylimepi/pi/models.json` generation |
...
- **Pi discovers `AGENTS.md` by walking up from `cwd`**, and also reads `agentDir`. Pass
  `agentsFilesOverride` or a sub-app inherits whatever is above `~/.keylimepi/apps/`.
...
- **Sessions default to `~/.pi/agent/`.** Pass an explicit `sessionDir` to
  `SessionManager.create/open/list` or transcripts land outside `~/.keylimepi/`.
