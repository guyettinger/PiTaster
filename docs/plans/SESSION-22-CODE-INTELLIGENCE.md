...
the self-modification flow relies on" — but that gate is anyapp's, run by a human. A
...
## Why anyapp's own, and not a package
...
which anyapp's React renderer can draw — and sized for cloud context windows.
...
exception is `apply_fix`'s `line`, which is the number anyapp printed in the diagnostics
...
5. **Guidance in `system-prompt.ts`, not on the tool definitions.** `systemPromptOverride`
   ...
   Pi's built-ins — so guidance on anyapp's own tools would be dead metadata.
