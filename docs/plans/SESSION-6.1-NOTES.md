...
Session 6.1 established the foundation for the sub-apps architecture by adding type definitions to `@keylimepi/core` and creating the `AppManager` class in `@keylimepi/shared` for sub-app lifecycle management.
...
| `AppMetadata` | Data stored in `.keylimepi-meta.json` |

## AppManager Methods

| Method | Purpose |
|--------|---------|
| `ensureAppsDir()` | Create `~/.keylimepi/apps/` if needed |
...
| `writeMetadata(appPath, meta)` | Write `.keylimepi-meta.json` |
