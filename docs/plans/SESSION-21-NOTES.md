...
`~/.anyapp/skills`; `read` is in `PATH_TOOLS`; `checkConfinement` refuses any path
...
bodies anyapp shipped, and a file is corrected — or deleted, where the honest correction
...
- Toggling `create-skill` off: header went 10 → 9 active and 620 → 553 tokens, exactly
  its 67-token entry; `disabledSkills` persisted to `.anyapp-meta.json`; the manifest
  dropped it.
...
**This session wrote to `~/.anyapp/`**: the seed migration corrected four skills and
