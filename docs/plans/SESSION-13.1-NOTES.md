...
1. **Inspector Overlay Script** (`packages/shared/src/inspector/overlay.ts`)
   ...
   - Global API exposed as `window.__anyappInspector`
...
5. **Preview Panel Integration** (`apps/electron/src/renderer/src/components/PreviewPanel.tsx`)
   ...
   - Message handler for `anyapp:element-selected` events
...
- Compiled as part of `@anyapp/shared` package build
...
- Event type: `anyapp:element-selected`
...
// Activate
await webview.executeJavaScript('window.__anyappInspector?.activate()')
```
