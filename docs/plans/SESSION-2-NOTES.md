...
**Plan:** Import types from `@clirabbit/core` across all Electron processes.

**Actual:** Defined types locally in each process (main, preload, renderer).

**Reason:** TypeScript module resolution with `moduleResolution: "bundler"` and workspace packages required building packages before type checking. While the types are defined in `@clirabbit/core` for external use, the Electron app uses local type definitions to avoid build ordering dependencies during development.
