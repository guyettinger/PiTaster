...
Session 5 completed the CLIRabbit application with enhanced chat UI, skills integration, settings panel, and polished sidebar navigation layout.
...
- `SkillsLoader` initialization with `~/.clirabbit/skills`
- `extractSkillMentions()` to parse `@skill-name` from prompts
- `buildSystemPrompt()` to inject skill content into base prompt

**Note:** The `@clirabbit/shared` package already had these utilities exported, so integration was straightforward.

### Part 3: Skills Files

**Plan:** Create skills at `~/.clirabbit/skills/`.
...
- Main config (theme, autoCommit) stored in `~/.clirabbit/config.json`
- API key stored separately in `~/.clirabbit/.apikey` using Electron's encrypted storage
...
2. **Skills directory location:** User-specific data like skills belongs in `~/.clirabbit/`, not the repo.
