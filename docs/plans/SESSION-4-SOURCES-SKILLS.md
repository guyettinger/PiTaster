...
import type { AnySourceConfig, ConnectedSource } from '@keylimepi/core'
...
```tsx
import { useState, useEffect } from 'react'
import type { ConnectedSource, McpSourceConfig } from '@keylimepi/core'
...
```typescript
import { SourceManager } from '@keylimepi/shared'
import { SkillsLoader } from '@keylimepi/shared'
import { homedir } from 'node:os'
import { join } from 'node:path'

const configDir = join(homedir(), '.Key Lime Pi')
...
Create these skills in `~/.keylimepi/skills/`:
...
---
name: self-modify
description: Modify the Key Lime Pi app's own source code safely. Use when the user wants to change app behavior, add features, or fix bugs.
---

# Self-Modification

When modifying Key Lime Pi's source code:
...
---
name: debug-fix
description: Debug issues and fix bugs in Key Lime Pi. Use when user reports errors or unexpected behavior.
---
