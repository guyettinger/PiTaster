...
Replace anyapp's hand-rolled Anthropic agent loop with [Pi](https://pi.dev/)
...
- **No API key, no network.** Inference is local. `~/.anyapp/.apikey` and the
...
anyapp keeps only what is genuinely its own — the permission modes, the git
...
anyapp domain tools over `isomorphic-git`, not coding built-ins, so they are ported to
...
- [ ] 15.2 — Settings lists locally pulled Ollama models; `~/.anyapp/pi/models.json` matches
