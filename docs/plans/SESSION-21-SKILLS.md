...
| Workspace | `~/.keylimepi/skills/` | **yes** | **no** | yes |
...
of those paths is under `~/.keylimepi/skills`, `read` is in `PATH_TOOLS`, and
...
contained. `self-modify` described Key Lime Pi's own monorepo, which the confined agent cannot
...
2. **Key Lime Pi's own manifest** — `renderSkillManifest`, naming `load_skill`. Pi's is
...
6. **Correct the seeds**, and migrate installs that still carry the bad copies —
   replacing only bodies that match byte-for-byte what Key Lime Pi shipped.
