...
| Workspace | `~/.pitaster/skills/` | **yes** | **no** | yes |
...
of those paths is under `~/.pitaster/skills`, `read` is in `PATH_TOOLS`, and
...
contained. `self-modify` described Pi Taster's own monorepo, which the confined agent cannot
...
2. **Pi Taster's own manifest** — `renderSkillManifest`, naming `load_skill`. Pi's is
...
6. **Correct the seeds**, and migrate installs that still carry the bad copies —
   replacing only bodies that match byte-for-byte what Pi Taster shipped.
