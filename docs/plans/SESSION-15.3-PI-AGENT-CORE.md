...
const { session } = await createAgentSession({
  cwd: app.path,
  agentDir: join(homedir(), '.Key Lime Pi', 'pi'),
  model,
  modelRuntime,
  thinkingLevel: 'off',
  noTools: 'all',
...
`input` and `output`. Keep the `@keylimepi/core` copy, delete the rest, add `toolCallId`:
...
`AUTHOR = { name: 'Key Lime Pi Agent', email: 'agent@keylimepi.local' }` (`agent.ts:22`).
...
Pi has no equivalent for Key Lime Pi's git tools, so they are ported rather than dropped.
...
**Skills** — keep Key Lime Pi's `SkillsLoader` (`~/.keylimepi/skills/<name>/SKILL.md`) and feed
the result through `DefaultResourceLoader({ skillsOverride })`, mapping `@keylimepi/core`'s
...
3. [ ] `default` mode + a write — approval prompt appears; approve; `git log` in
       `~/.keylimepi/apps/<id>` shows the commit
...
- StreamChunk is single-source in @keylimepi/core and gains toolCallId, fixing
