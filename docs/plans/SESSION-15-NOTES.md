...
local Ollama models, with Key Lime Pi's permission modes, path confinement, git auto-commit,
...
6. **`apps/electron/src/main/agent/ollama.ts`** (~275 lines) — model discovery and
   `~/.keylimepi/pi/models.json` generation.
...
Key Lime Pi's UI creates a visible, selectable session the moment the user clicks New Chat.
...
`@keylimepi/shared`: TypeScript with `moduleResolution: "bundler"` emits extensionless
...
| Selecting a model persists to `~/.keylimepi/config.json` | ✅ |
...
confinement to `~/.keylimepi/apps/`. Worth a follow-up.
