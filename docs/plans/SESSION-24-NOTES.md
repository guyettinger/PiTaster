...
1. **`apps/electron/src/main/session-baselines.ts`**
   ...
   - Stored at `~/.keylimepi/session-baselines.json`, beside `layouts.json`
...
2. **`apps/electron/src/renderer/src/hooks/useSessionChanges.ts`**
   ...
   - Filters Key Lime Pi's own bookkeeping files out of the result
...
layouts do. `.keylimepi-meta.json` is tracked and `initGitRepo` adds every file, so
...
**`.keylimepi-meta.json` sits permanently modified.** It is tracked — `initGitRepo`
