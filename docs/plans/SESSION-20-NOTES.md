...
`replace_lines: README.md`. Deleting `~/.keylimepi/skills` and relaunching reinstated all
eight skills.
...
with Pi. Copying them into Key Lime Pi's prompt would have fixed the symptom and created a
...
higher on the models Key Lime Pi targets.
...
entirely. Key Lime Pi has supplied a custom prompt since Pi was adopted, so the model had
...
`~/.keylimepi/apps/`, so `~/.keylimepi/AGENTS.md` and `~/AGENTS.md` were entering every
