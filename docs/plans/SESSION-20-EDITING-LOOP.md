...
session without whitespace roulette — by giving back the editing guidance Key Lime Pi
...
### 1. Key Lime Pi discards Pi's editing guidance
...
Key Lime Pi supplies `systemPromptOverride`, so `buildSystemPrompt` takes its
...
bullets is discarded, for every tool. Key Lime Pi's own prompt says only "prefer
...
- **Nothing in Key Lime Pi reacts to a failed edit.** `agent/loop-guard.ts` blocks a
...
**Runtime skills are never installed.** `loadPiSkills` reads `~/.keylimepi/skills`
...
Key Lime Pi passes neither `noContextFiles` nor `agentsFilesOverride`, so a stray
`~/AGENTS.md` or `~/.keylimepi/AGENTS.md` silently enters every session — unbounded
...
qwen builds Key Lime Pi targets. Reproducing a code region's exact whitespace is
