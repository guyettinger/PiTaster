...
Then open **Settings → General** and pick your model. anyapp asks the daemon what
...
| `bun run --filter @anyapp/electron dev` | Run a single workspace |
...
Everything is under `~/.anyapp/`:

| Path | What's in it |
|------|--------------|
| `~/.anyapp/apps/` | Your sub-apps — each one a real project with its own git repo |
| `~/.anyapp/skills/` | Workspace skills, offered to every app |
| `~/.anyapp/pi/` | Model configuration and Pi's chat transcripts |
...
actually served. anyapp discovers the real number by warming the model and
