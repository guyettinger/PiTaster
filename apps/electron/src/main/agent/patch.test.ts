 ...
 * "produces a nice diff" but **accurate or absent**: every case where Key Lime Pi cannot know
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'keylimepi-patch-'))
})
...
describe('previewPatch — absent rather than wrong', () => {
  test('says nothing when an edit’s text is not in the file', async () => {
    ...
    // Pi's matcher may still land this through its fuzzy fallback. Key Lime Pi does not
