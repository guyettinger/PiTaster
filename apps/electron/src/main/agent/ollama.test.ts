...
describe('writeOllamaModelsFile', () => {
  ...
  async function agentDir(): Promise<string> {
    return mkdtemp(join(tmpdir(), 'keylimepi-models-'))
  }
  ...
  test('preserves a provider Key Lime Pi did not write', async () => {
  ...
  test('replaces a file that is not JSON', async () => {
    // A file Pi cannot parse is worse than one Key Lime Pi overwrote.
    const dir = await agentDir()
