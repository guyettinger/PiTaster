 ...
 * and leaves the model's own Modelfile default alone; `'auto'` asks Key Lime Pi to choose
 ...
 * baked into the config was how Key Lime Pi came to run every thinking model greedily.
 */
...
/** Default for both settings: let Key Lime Pi choose from the model. */
 ...
 * both, and Key Lime Pi shipped one number.
 *
...
export function resolveSampling(params: ResolveSamplingParams): ResolvedSampling {
  ...
  if (typeof topP === 'number') {
    ...
    // do anything — and an install carrying Key Lime Pi's old pinned 0 would have got
