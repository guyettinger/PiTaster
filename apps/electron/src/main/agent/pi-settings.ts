/**
 * The settings anyapp layers over Pi's, and the loader that makes them stick.
 *
...
// enforces the streaming half of it. Pi DOES read this setting on the path anyapp
...
/**
 * A resource loader that puts anyapp's settings back after every reload.
 *
 ...
 * override anyapp applied became `{}` and Pi fell back to its own defaults. Applying
 ...
 * anyapp had configured. Compaction thresholds and the retry policy were lost with it.
 *
 * Re-applying AFTER the reload rather than before is what makes this hold no matter
 * who reloads.
 */
export class AnyappResourceLoader extends DefaultResourceLoader {
  /**
   * @param options - Pi's own loader options
   * @param reapplySettings - Puts anyapp's overrides back on the settings manager
   */
  constructor(
