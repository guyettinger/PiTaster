 ...
 *
 * anyapp's safety story is that every write is auto-committed so any change can be
 ...
 * write knowing only its path, which is the one place in anyapp where a person is asked
 ...
 * - `replace_lines` is anyapp's own and purely positional, so the diff is exact.
