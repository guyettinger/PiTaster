 ...
 * Key Lime Pi's own dependency tree rather than the user's, and without which nothing type
 * checks at all.
 *
 ...
 *
 * Deliberately permissive. An inferred config means Key Lime Pi is guessing at the project's
 ...
 * here, and more sharply — a `tsconfig.json` above `~/.keylimepi/apps/` would silently
...
export function createTsProject(rootPath: string): TsProject {
  ...
  // Key Lime Pi's dependency tree, not the user's project, so it is ours rather than content
