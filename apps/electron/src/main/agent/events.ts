/**
 * Adapts Pi's session event stream to Key Lime Pi's {@link StreamChunk} protocol.
 */

import type { AgentSessionEvent } from '@earendil-works/pi-coding-agent'
import type { AgentStatus, FilePatch, StreamChunk } from '@keylimepi/core'
 ...
 *
 * `details` is where Key Lime Pi puts everything the UI needs and the model must not pay
...
export function toStreamChunk(event: AgentSessionEvent): StreamChunk | null {
  switch (event.type) {
    case 'message_update': {
      ...
      // Ollama's models reason on every request whatever Key Lime Pi asks for — the
