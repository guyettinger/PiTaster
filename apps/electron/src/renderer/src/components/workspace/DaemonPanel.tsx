    ...
    <div className="h-full overflow-y-auto bg-panel [&>section]:max-w-md">
      ...
      <section className="border-b border-line px-3 py-3">
        ...
        <p className="mt-1.5 text-[11px] text-ash">
          Medians over this session&rsquo;s requests. A rate anyapp has not measured is
 ...
 * what the daemon sized it to, so a window anyapp had to fall back on is usually much
...
function describeSource(source: ContextReport['windowSource']): string {
  switch (source) {
    ...
    case 'fallback':
      // Deliberately about what anyapp knows, not about what the daemon is doing.
      ...
      return 'A conservative default — anyapp has not read a window figure from the daemon for this model. The real one is measured when the next turn warms it.'
  }
}
